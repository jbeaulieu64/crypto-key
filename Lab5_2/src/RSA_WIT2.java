...
public class RSA_WIT2 {
	public static void main(String[] args) {
		...
		RSA_WIT2 rsa = new RSA_WIT2("977447",  "649487", "6359");		// see the constructor for details.
		//  p            q     publicKey
	 ...
	 *
	 */
	public RSA_WIT2(String the_p, String the_q, String the_public_key) {
	...
	*/
	...
	public void EncryptFile(String inf, String outf) {
		...
		try {
			out = new FileOutputStream(new File(outf));
			in  = new FileInputStream(new File(inf));
			length = new File(inf).length();
		}
		catch(IOException e) {
			System.err.println(e);
			System.exit(-1);
		}
			// remainder number of left over bytes
			// remainder encrypt then written to file
		BigInteger remainder = new BigInteger(String.valueOf(length % NB));
		Write_BigInteger_to_file(out, encrypt(remainder));
		int w = 0;
		byte[] buffer = new byte[NB];
	...
	public void DecryptFile(String fname, String outfile) {
		...
		int modulusSize = modulus.bitLength()/8 + 1;	// calculate how many BYTES the module is (modulusSize).
		byte [] dataBuffer = new byte[modulusSize];		// modulus is 5 bytes (40 bits) - need one more byte
		// dataBuffer will hold (modulusSize) number of bytes from the encrypted file.
		try {
			in.read(dataBuffer);
			BigInteger tmp = new BigInteger(dataBuffer);	// convert the encrypted bytes into a BigInteger
			int remainderSize = decrypt(tmp).intValue();

			int ret = -1;
			while ( (ret = in.read(dataBuffer)) == modulusSize ) {	// go in a loop and read (modulusSize) number of bytes from the encrypted file.
				BigInteger enc = new BigInteger(dataBuffer);	// convert the encrypted bytes into a BigInteger
				...
				if( data.length > NB ) {				// dump only the last NB bytes
					...
					for(int r=NB-1; r>=0; r--)	gg[r] = data[r];
					...
					if( in.available() > 0 ) {	// if there is more data in the encrypted file, 
						...
						byte[] gg = new byte[NB];
						for(int r=0; r<NB; r++)	gg[r] = (byte)0;
						for(int r=NB-1, r2 = data.length-1; r>=0 && r2 >= 0; r--, r2--)
							gg[r] = data[r2];
						out.write(gg);

					} else {
						if (data.length < remainderSize) {
							byte[] bytes = new byte[remainderSize];
							for (int i = 0; i < remainderSize; i++) { bytes[i] = 0; }
							for (int j = remainderSize-1, k = data.length-1; j >= 0 && k >= 0; j--, k--) {
								bytes[j] = data[k];
							}
